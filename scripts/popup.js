...
byId('downloads').onclick = function (e) {
	...
	if (e.target.nodeName == 'A') {
		if (!/resume|cancel|pause|retry|erase|name|show|deletefile/.test(c)){
			return;
		};
...
function get_download_view(e) {
		...
		var controls = "", status = "", progress_class = "", deletefile = "", progress_width = 0;
		if (e.state == 'complete') { 
			...
			if (!e.exists) { 
				...
				deletefile = chrome.i18n.getMessage("item_delete_file_button_title");
			} 
		...
		return "<div class='download " + extra_class + "' id='download-" + e.id + "' data-id='"+ e.id +"' draggable='"+ draggable_attr +"'>" +
			        ...
			        "<div class='controls'>" + controls  + "<span class='status'>" + status + "</span>" + "<a class='deletefile' href='#'>" + deletefile + "</a>" +"</div>" +
		        "</div>";
